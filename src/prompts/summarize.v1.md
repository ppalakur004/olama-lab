...
when the source does not provide the field, use
{"value": null, "status": "absent", "citation": null}

when the source is conflicting or unclear, keep "value" present (null or the conflicting strings)
and use status "ambiguous"

do not invent a citation

do not add fields that are not in the supplied schema. Meeting-note headings such as
attendees or discussion must not become JSON keys.
...
If the marked text is not an applicable procedure (meeting notes, newsletter, agenda),
set document_status to "unsupported".

Still return every listed field. Use {"value": null, "status": "absent", "citation": null}
for procedure slots the source does not support.
